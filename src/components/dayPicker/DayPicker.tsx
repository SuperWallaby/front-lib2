...
=======
import React, { Fragment, useRef, useEffect } from 'react';
import { CaptionElementProps, DayModifiers, DayPickerProps } from 'react-day-picker';
const DayPicker = require('react-day-picker').default;
import classNames from 'classnames';
import Caption from './component/Caption';
import Navbar from './component/Navbar';
...
=======
		const horizenProps =
			mode === 'horizen'
				? {
						...
						numberOfMonths: 3,
						showWeekDays: false,
						captionElement: ({ date }: CaptionElementProps) => <HorizenCaption date={date} onChange={() => {}} />
					}
				: {};
		...
		const dayPickerProps: DayPickerProps = {
			...
			initialMonth: moment(from || new Date()).toDate(),
			showWeekDays: true,
			captionElement: ({ date }: CaptionElementProps) => <Caption displayYear={displayYear} date={date} />,
			navbarElement: <Navbar />,
			className: `Range ${classes}`,
			selectedDays,
			modifiers,
